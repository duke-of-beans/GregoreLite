/**
 * GET /api/recall/active — returns the currently surfaced recall event (if any)
 * Sprint 27.0
 */
...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    // Active = surfaced within last 4 hours, not yet acted on
    const fourHoursAgo = Date.now() - 4 * 60 * 60 * 1000;

    const row = db.prepare<[number], EventRow>(`
      ...
      LIMIT 1
    `).get(fourHoursAgo) as EventRow | undefined;
