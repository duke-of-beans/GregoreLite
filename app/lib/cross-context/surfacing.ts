/**
 * Proactive surfacing — ranking, filtering, suppression (Sprint 3E)
 *
 * Ranking formula: score = similarity² × recencyFactor × (1 - dismissalPenalty)
 * Max 2 suggestions visible. Min display score: 0.70.
 * Suppression: 3 dismissals → 48h; 5 dismissals in 7 days → 7-day.
 *
...
export function getDismissalPenalty(chunkId: string): number {
  ...
  const windowStart = Date.now() - DISMISSAL_WINDOW_MS;
  ...
  return Math.min(row.count * DISMISSAL_PENALTY_PER, DISMISSAL_MAX_PENALTY);
}
...
export function isSuppressed(chunkId: string): boolean {
  ...
  // 7-day suppression: 5+ dismissals in 7-day window
  const window7d = Date.now() - SUPPRESSION_WINDOW_7D_MS;
  const row7d = db.prepare(`
    SELECT COUNT(*) as count FROM suggestions
    WHERE chunk_id = ? AND user_action = 'dismissed' AND acted_at > ?
  `).get(chunkId, window7d) as { count: number };
  if (row7d.count >= SUPPRESSION_5_THRESHOLD) return true;

  // 48h suppression: 3+ dismissals in 48h window
  const window48h = Date.now() - SUPPRESSION_WINDOW_48H_MS;
  const row48h = db.prepare(`
    SELECT COUNT(*) as count FROM suggestions
    WHERE chunk_id = ? AND user_action = 'dismissed' AND acted_at > ?
  `).get(chunkId, window48h) as { count: number };
  if (row48h.count >= SUPPRESSION_3_THRESHOLD) return true;
...
export function rankAndFilter(
  ...
  for (const candidate of candidates) {
    ...
    const chunk = db
      ...
      .get(candidate.chunkId) as ChunkMetaRow | undefined;
    if (!chunk) continue;
